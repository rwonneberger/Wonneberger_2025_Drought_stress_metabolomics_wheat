# Use the statgenHTP package to filter the imaging data time series dataset and remove outliers in Experiment 2
...
setwd("//storage-al.slu.se/home$/rawo0001/My Documents/Projects/CresWheat/Analyses/CresWheat/")

## time should be in YYYY-MM-DD timepoint 

NORDrought <- read.csv("//storage-al.slu.se/home$/rawo0001/My Documents/Projects/CresWheat/Data/2132TB/NORDrome_analysis_report.csv", header = TRUE, sep = ",")
...
NORDrought%<>%filter(Genotype != "empty pot")
...
directory="Results/LemnaTecPhenotyping/"
...
for (i in 1:length(trait_list)){
  ...
  plot(phenoTPc, 
       plotType = "cor",
       traits = trait,
       timePoints = seq(from = 1, to = 29, by = 1))
}
dev.off()
...
for (i in 1:length(trait_list)){
  for (j in 1:length(trt_list)){
    ...
    # We don't want to remove outliers around end of drought/rewatering , so we set outlier=1 after 01.09. back to 0
    resuVatorHTP[(resuVatorHTP$timePoint == as.POSIXct("2021-09-01", "UTC") | resuVatorHTP$timePoint == as.POSIXct("2021-09-02", "UTC") |  resuVatorHTP$timePoint == as.POSIXct("2021-09-03", "UTC") | resuVatorHTP$timePoint == as.POSIXct("2021-09-04", "UTC") |  resuVatorHTP$timePoint == as.POSIXct("2021-09-05", "UTC") | resuVatorHTP$timePoint == as.POSIXct("2021-09-06", "UTC") |  resuVatorHTP$timePoint == as.POSIXct("2021-09-07", "UTC") |  resuVatorHTP$timePoint == as.POSIXct("2021-09-08", "UTC")) & resuVatorHTP$outlier == 1, ]$outlier<-0
    ...
    # We don't want to remove outliers around end of drought/rewatering , so we set outlier=1 after 01.09. back to 0
    resuVatorHTP1[(resuVatorHTP1$timePoint == as.POSIXct("2021-09-01", "UTC") | resuVatorHTP1$timePoint == as.POSIXct("2021-09-02", "UTC") |  resuVatorHTP1$timePoint == as.POSIXct("2021-09-03", "UTC") | resuVatorHTP1$timePoint == as.POSIXct("2021-09-04", "UTC") |  resuVatorHTP1$timePoint == as.POSIXct("2021-09-05", "UTC") | resuVatorHTP1$timePoint == as.POSIXct("2021-09-06", "UTC") |  resuVatorHTP1$timePoint == as.POSIXct("2021-09-07", "UTC") |  resuVatorHTP1$timePoint == as.POSIXct("2021-09-08", "UTC")) & resuVatorHTP1$outlier == 1, ]$outlier<-0
    ...
    # We don't want to remove outliers around end of drought/rewatering , so we set outlier=1 after 01.09. back to 0
    resuVatorHTP2[(resuVatorHTP2$timePoint == as.POSIXct("2021-09-01", "UTC") | resuVatorHTP2$timePoint == as.POSIXct("2021-09-02", "UTC") |  resuVatorHTP2$timePoint == as.POSIXct("2021-09-03", "UTC") | resuVatorHTP2$timePoint == as.POSIXct("2021-09-04", "UTC") |  resuVatorHTP2$timePoint == as.POSIXct("2021-09-05", "UTC") | resuVatorHTP2$timePoint == as.POSIXct("2021-09-06", "UTC") |  resuVatorHTP2$timePoint == as.POSIXct("2021-09-07", "UTC") |  resuVatorHTP2$timePoint == as.POSIXct("2021-09-08", "UTC")) & resuVatorHTP2$outlier == 1, ]$outlier<-0
...
for (i in 1:length(subdir_list)){
  control_list[[i]]<-(paste0(directory, subdir_list[i], "/", subdir_list[i], "drought_noout.txt"))
}
...
df00$Replicate<-NULL
fwrite(df00, paste0(directory, "All_drought_noout.txt"), sep="\t")



###################################
# Add an SD filter, and remove reps with more than x missing datapoints
df<-fread("Results/LemnaTecPhenotyping/All_drought_noout.txt")
...
d1_long<-d1_long%>%filter(genotype == "PPPW_034", variable == "hsv_h_brown2green")
...
d2<-df_filtered_long%>%filter(genotype == "PPPW_034", variable == "hsv_h_brown2green")
...
fwrite(df_filtered, paste0(directory, "All_drought_sd_NA_filt.txt"), sep="\t")
